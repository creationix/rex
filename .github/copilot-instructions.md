...
- Prefer validating parser/encoder behavior with `bun run rex:compile --expr "..." --ir` before changing tests.
...
- If parser/IR/encoder/interpreter changed: run `bun test` in `packages/rex-lang` and spot-check with `bun run rex:compile --expr ... --ir`.
