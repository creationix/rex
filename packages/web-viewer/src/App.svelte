...
>
	...
	<div class="flex flex-1 min-h-0">
		...
		<main
			class="flex-1 min-w-0 relative {appState.mode === 'split' ? 'flex' : ''}"
		>
			...
			{:else if appState.mode === "split"}
				<div
					class="lg:hidden absolute top-0 left-0 right-0 z-20 flex items-center gap-1 border-b border-[#333] bg-[#111] px-2 py-1.5"
				>
					<button
						type="button"
						onclick={() => {
							appState.activePane = "data";
						}}
						class="text-[11px] px-2 py-1 rounded border {appState.activePane ===
						'data'
							? 'border-white text-white'
							: 'border-[#333] text-[#888]'}">Data Pane</button
					>
					<button
						type="button"
						onclick={() => {
							appState.activePane = "encoding";
						}}
						class="text-[11px] px-2 py-1 rounded border {appState.activePane ===
						'encoding'
							? 'border-white text-white'
							: 'border-[#333] text-[#888]'}">Encoding Pane</button
					>
				</div>
				<div
					bind:this={dataPane}
					class="min-w-0 flex-1 {appState.activePane === 'data'
						? 'block pt-9 lg:pt-0'
						: 'hidden lg:block'}"
				>
					<DataTreeView />
				...
				<div
					...
					class="min-w-0 flex-1 {appState.activePane === 'encoding'
						? 'block pt-9 lg:pt-0'
						: 'hidden lg:block'}"
				>
					<EncodingView />
			...
			{#if appState.error}
				<div
					class="absolute bottom-0 left-0 right-0 px-4 py-2 bg-[#2a1215] border-t border-[#f4877133] text-xs text-[#f48771]"
				>
					{appState.error}
				</div>
			{/if}
		...
		{#if dragging}
			<div
				class="absolute inset-0 z-50 flex items-center justify-center bg-[#000c] border-2 border-dashed border-[#555] pointer-events-none"
			>
