...
<div class="flex flex-col h-screen" ondragover={handleDragOver} ondragleave={handleDragLeave} ondrop={handleDrop}>
	...
	<div class="flex flex-1 min-h-0">
		...
		<main class="flex-1 min-w-0 relative">
			{#if appState.mode === 'source'}
				<SourceView />
			{:else if appState.mode === 'encoding'}
				<EncodingView />
			{:else if appState.mode === 'data'}
				<DataView />
			{/if}
			{#if appState.error && appState.mode !== 'source'}
		...
		{/if}
		{#if appState.refsOpen}
			<div class="w-[300px] shrink-0 border-l border-[#222]">
				<RefsView />
			</div>
		{/if}
	</div>
