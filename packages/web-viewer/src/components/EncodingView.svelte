<script lang="ts">
	...
	const gutterDigits = $derived(
		rootNode ? Math.max(1, b64sizeof(rootNode.right)) : 1,
	);
	function fmtOffset(n: number): string {
		return b64(n).padStart(gutterDigits, "0");
	}
	...
	const isActive = $derived(
		appState.mode !== "split" || appState.activePane === "encoding",
	);
	...
	function jumpToOffset(targetOffset: number, pushHistory: boolean = true) {
		...
		if (pushHistory && focusIdx != null && focusIdx < rows.length) {
			jumpHistory.push({
				nodeRight: rows[focusIdx].node.right,
		...
		if (
			row &&
			isContainerTag(row.node.tag) &&
			!appState.isOpened(row.node.right)
		) {
	...
	function expandPathTo(node: ASTNode, targetOffset: number): boolean {
		...
		if (targetOffset < node.left - node.size || targetOffset > node.right)
			return false;
		for (const child of node) {
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if (e.key === "Tab" && appState.mode === "split") {
			e.preventDefault();
			appState.activePane =
				appState.activePane === "encoding" ? "data" : "encoding";
			return;
		}
	...
	function handleClick(e: MouseEvent) {
		...
		if (focusIdx != null && focusIdx !== idx) {
			...
			if (focusedRow) {
				jumpHistory.push({
					nodeRight: focusedRow.node.right,
					scrollTop: viewport?.scrollTop ?? 0,
				});
			}
		}
	...
	async function copyAsJson() {
		...
		try {
			await navigator.clipboard.writeText(
				JSON.stringify(ctxMenu.node.value, null, 2),
			);
		} catch {}
		ctxMenu = null;
	}
	...
	$effect(() => {
		return appState.onFocusSync((nodeRight, source) => {
			if (source !== "encoding" && appState.mode === "split")
	...
	$effect(() => {
		return appState.onExpandChange((nodeRight, expanded) => {
			if (appState.mode !== "split" || appState.activePane === "encoding")
				return;
						...
						{#each visibleRows as row, i (visibleStart + i)}
							...
							>
								...
								>
									...
									<!-- Collapsed summary -->
									{#if isC && !row.opened}
										<span class="ml-1 text-[11px]" style="color: {DIM_COLOR}"
											>…</span
										>
									{/if}
								</div>
